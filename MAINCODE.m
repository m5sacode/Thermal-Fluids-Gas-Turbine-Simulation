...
title('Mass Flow Rates vs Inlet Temperature')
legend('Inlet','Outlet')
grid on
...
ylabel('Mass Flow Rate [ lbm/hr ]')
title('Mass Flow Rates vs Fuel Mass Flow Rate')
legend('Inlet','Outlet')
grid on
...
% extra plot without 20%
% specific fuel consumption
figure
subplot(2,1,1)
plot(percent(:,2:end) * 100, SFC(:,2:end), '.-', MarkerSize = 20)
xlabel('% Base Case Fuel Rate')
ylabel('SFC [ lbm/kW-hr ]')
title('Specific Fuel Consumption vs Fuel Mass Flow Rate')
grid on
...
% heat rate
subplot(2,1,2)
plot(percent(:,2:end) * 100, HR(:,2:end), '.-', MarkerSize = 20)
xlabel('% Base Case Fuel Rate')
ylabel('HR [ BTU/kW-hr ]')
title('Heat Rate vs Fuel Mass Flow Rate')
grid on
